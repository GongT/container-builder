import { DeferredPromise, definePublicConstant, registerGlobalLifecycle, toDisposable } from '@idlebox/common';
...
export function registerAuto(): ClassDecorator {
	return (target: any) => {
		...
		if (!token) throw new Error(`can't auto register dependency, unknown class: ${target.name}`);
...
function getCreateService<T>(paths: string[], token: IDependencyToken<T>) {
	...
	if (!Class) throw new Error('missing dependency: ' + tokenStr);
...
async function initInstance(token: IDependencyToken<any, any>, state: IState, props: any[]) {
	// console.error('[DI] <%s> finalize:', token.toString());
	if (state.fullfilled) throw new Error('duplicate finalize: ' + token.toString());
...
export function resolveService<T>(token: IDependencyToken<T>): MaybePromise<CanNotInit<T>> {
	if (!token) throw new Error('invalid token (likely circular import)');

	const state = getCreateService<T>([], token);

	return state.fullfilled ? state.instance : state.dfd.p;
}

export async function createInstance<T>(token: TokenTypeOf<T>, ...props: ArgTypeOf<T>): Promise<T> {
	const Class = container.get(token);
	if (!Class) throw new Error('missing dependency: ' + token.toString());
	const instance = new Class();
	const dfd = new DeferredPromise<void>();
...
export function inject<T>(token: TokenTypeOf<T>, ...args: ArgTypeOf<T>): InjectDecorator {
	if (args.some((e: any) => e !== undefined)) throw new Error('not impl: inject with args');

	if (!token) throw new Error('invalid token (likely circular dependency)');
